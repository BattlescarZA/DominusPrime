...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.dominusprime', 'whatsapp', 'session'));
...
const DEFAULT_REPLY_PREFIX = '🤖 *DominusPrime*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['DominusPrime Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore DominusPrime's own reply messages in self-chat mode to avoid loops.
