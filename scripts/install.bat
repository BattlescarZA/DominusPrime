...
REM dominusprime Installer for Windows (cmd.exe / batch)
REM Usage: install.bat [-Version X.Y.Z] [-FromSource] [-SourceDir DIR]
REM                         [-Extras "llamacpp,mlx"] [-UvPath PATH] [-Help]
REM
REM Installs dominusprime into %USERPROFILE%\.dominusprime with a uv-managed Python environment.
...
REM ── Defaults ──────────────────────────────────────────────────────────────────
if defined dominusprime_HOME (
    set "dominusprime_HOME=%dominusprime_HOME%"
) else (
    set "dominusprime_HOME=%USERPROFILE%\.dominusprime"
)
...
REM ──── Help ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
:show_help
echo dominusprime Installer for Windows
echo.
...
:write_warn
echo [dominusprime] WARNING: %~1
exit /b 0

:write_err
echo [dominusprime] ERROR: %~1
exit /b 0
...
echo [dominusprime] Downloading uv ^(!_DL_ARCH!^) from GitHub Releases...
...
if errorlevel 1 (
    echo [dominusprime] ERROR: GitHub download also failed.
    echo [dominusprime] Download uv manually from: https://github.com/astral-sh/uv/releases/latest
    del "!_DL_ZIP!" >nul 2>&1
    exit /b 1
)
...
echo [dominusprime] Extracting uv...
powershell -NoProfile -Command "Expand-Archive -Force -Path '!_DL_ZIP!' -DestinationPath '!_DL_DEST!'"
set "_DL_ERR=%errorlevel%"
del "!_DL_ZIP!" >nul 2>&1
if %_DL_ERR% neq 0 (
    echo [dominusprime] ERROR: Extraction failed.
    exit /b 1
)
if not exist "!_DL_DEST!\uv.exe" (
    echo [dominusprime] ERROR: uv.exe not found after extraction.
    exit /b 1
)
set "PATH=!_DL_DEST!;!PATH!"
echo [dominusprime] uv installed: !_DL_DEST!\uv.exe
exit /b 0
...
if defined ARG_UV_PATH (
    if not exist "%ARG_UV_PATH%" (
        echo [dominusprime] ERROR: Specified uv not found: %ARG_UV_PATH%
        exit /b 1
    )
    for %%I in ("%ARG_UV_PATH%") do set "PATH=%%~dpI;!PATH!"
    echo [dominusprime] uv found: %ARG_UV_PATH%
    goto :ensure_uv_done
)
...
for %%c in ("%USERPROFILE%\.local\bin\uv.exe" "%USERPROFILE%\.cargo\bin\uv.exe" "%LOCALAPPDATA%\uv\uv.exe") do (
    if exist %%c (
        ...
        echo [dominusprime] uv found: %%~c
        goto :ensure_uv_done
    )
)

REM 3. Try astral.sh (standard installer, fast outside China)
echo [dominusprime] Installing uv via astral.sh...
...
echo [dominusprime] astral.sh unreachable, falling back to GitHub Releases...
call :download_uv_github
if errorlevel 1 (
    echo [dominusprime] ERROR: Failed to install uv automatically.
    echo [dominusprime] Please install uv manually: https://docs.astral.sh/uv/
    exit /b 1
)
goto :ensure_uv_done
...
where uv >nul 2>&1
if errorlevel 1 (
    echo [dominusprime] ERROR: Failed to install uv. Please install it manually: https://docs.astral.sh/uv/
    exit /b 1
)
echo [dominusprime] uv installed via astral.sh
...
set "_CONSOLE_DEST=%_REPO_DIR%\src\dominusprime\console"
...
if exist "%_CONSOLE_SRC%\index.html" (
    echo [dominusprime] Copying console frontend assets...
...
if not exist "%_REPO_DIR%\console\package.json" (
    echo [dominusprime] WARNING: Console source not found - the web UI won't be available.
    exit /b 0
)

where npm >nul 2>&1
if errorlevel 1 (
    echo [dominusprime] WARNING: npm not found - skipping console frontend build.
    echo [dominusprime] WARNING: Install Node.js from https://nodejs.org/ then re-run this installer,
    echo [dominusprime] WARNING: or run 'cd console ^&^& npm ci ^&^& npm run build' manually.
    exit /b 0
)

echo [dominusprime] Building console frontend (npm ci ^&^& npm run build)...
pushd "%_REPO_DIR%\console"
npm ci
if errorlevel 1 (
    popd
    echo [dominusprime] WARNING: npm ci failed - the web UI won't be available.
    exit /b 0
)
npm run build
if errorlevel 1 (
    popd
    echo [dominusprime] WARNING: npm run build failed - the web UI won't be available.
    exit /b 0
)
popd
...
if exist "%_CONSOLE_SRC%\index.html" (
    ...
    set "CONSOLE_COPIED=1"
    set "CONSOLE_AVAILABLE=1"
    echo [dominusprime] Console frontend built successfully
    exit /b 0
)

echo [dominusprime] WARNING: Console build completed but index.html not found - the web UI won't be available.
exit /b 0
...
REM %~1 = RepoDir
if "%CONSOLE_COPIED%"=="1" (
    set "_CLEANUP_DEST=%~1\src\dominusprime\console"
...
REM ══════════════════════════════ MAIN ═════════════════════════════════════════
:main
echo [dominusprime] Installing dominusprime into %dominusprime_HOME%
...
if exist "%dominusprime_VENV%" (
    ...
    echo [dominusprime] Creating Python %PYTHON_VERSION% environment...
)

uv venv "%dominusprime_VENV%" --python %PYTHON_VERSION% --quiet --clear
if errorlevel 1 (
    echo [dominusprime] ERROR: Failed to create virtual environment
    exit /b 1
)

set "VENV_PYTHON=%dominusprime_VENV%\Scripts\python.exe"
...
echo [dominusprime] Installing dominusprime from local source: %ARG_SOURCE_DIR%
call :prepare_console "%ARG_SOURCE_DIR%"
echo [dominusprime] Installing package from source...
...
if not exist "%VENV_dominusprime%" (
    echo [dominusprime] ERROR: Installation failed: dominusprime CLI not found in venv
    exit /b 1
)
echo [dominusprime] dominusprime installed successfully

REM Check console availability (for PyPI installs, probe the installed package)
if "%CONSOLE_AVAILABLE%"=="0" (
    "%VENV_PYTHON%" -c "import importlib.resources, dominusprime; p=importlib.resources.files('dominusprime')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" > "%TEMP%\_dominusprime_console_check.tmp" 2>&1
    set /p CONSOLE_CHECK=<"%TEMP%\_dominusprime_console_check.tmp"
    del "%TEMP%\_dominusprime_console_check.tmp" >nul 2>&1
...
set "WRAPPER_PS1=%dominusprime_BIN%\dominusprime.ps1"
echo # dominusprime CLI wrapper -- delegates to the uv-managed environment. > "%WRAPPER_PS1%"
echo $ErrorActionPreference = "Stop" >> "%WRAPPER_PS1%"
echo. >> "%WRAPPER_PS1%"
echo $dominusprimeHome = if ($env:dominusprime_HOME) { $env:dominusprime_HOME } else { Join-Path $HOME ".dominusprime" } >> "%WRAPPER_PS1%"
echo $RealBin = Join-Path $dominusprimeHome "venv\Scripts\dominusprime.exe" >> "%WRAPPER_PS1%"
echo. >> "%WRAPPER_PS1%"
echo if (-not (Test-Path $RealBin)) { >> "%WRAPPER_PS1%"
echo     Write-Error "dominusprime environment not found at $dominusprimeHome\venv" >> "%WRAPPER_PS1%"
echo     Write-Error "Please reinstall: irm ^<install-url^> ^| iex" >> "%WRAPPER_PS1%"
echo     exit 1 >> "%WRAPPER_PS1%"
echo } >> "%WRAPPER_PS1%"
echo. >> "%WRAPPER_PS1%"
echo ^& $RealBin @args >> "%WRAPPER_PS1%"
echo [dominusprime] Wrapper created at %WRAPPER_PS1%

REM CMD wrapper
set "WRAPPER_CMD=%dominusprime_BIN%\dominusprime.cmd"
echo @echo off > "%WRAPPER_CMD%"
echo REM dominusprime CLI wrapper -- delegates to the uv-managed environment. >> "%WRAPPER_CMD%"
echo set "dominusprime_HOME=%%dominusprime_HOME%%" >> "%WRAPPER_CMD%"
echo if "%%dominusprime_HOME%%"=="" set "dominusprime_HOME=%%USERPROFILE%%\.dominusprime" >> "%WRAPPER_CMD%"
echo set "REAL_BIN=%%dominusprime_HOME%%\venv\Scripts\dominusprime.exe" >> "%WRAPPER_CMD%"
echo if not exist "%%REAL_BIN%%" ( >> "%WRAPPER_CMD%"
echo     echo Error: dominusprime environment not found at %%dominusprime_HOME%%\venv ^>^&2 >> "%WRAPPER_CMD%"
...
set "check_str=;%dominusprime_BIN%;"
if /i "%path_check%" neq "%path_check:%check_str%=%" (
    echo [dominusprime] %dominusprime_BIN% already in PATH
) else (
    :: === 修复1：安全传递参数（解决命令注入） ===
    if defined CURRENT_USER_PATH (
        powershell -NoProfile -Command "$p = $args[0]; $v = $args[1]; [Environment]::SetEnvironmentVariable('Path', $p + ';' + $v, 'User')" "%dominusprime_BIN%" "!CURRENT_USER_PATH!"
    ) else (
        powershell -NoProfile -Command "$p = $args[0]; [Environment]::SetEnvironmentVariable('Path', $p, 'User')" "%dominusprime_BIN%"
    )

    :: === 修复2：添加关键错误检查（解决失败不报错） ===
    if errorlevel 1 (
        echo [error] Failed to update PATH. dominusprime_BIN: "%dominusprime_BIN%"
        echo [error] Please verify the path is valid.
        exit /b 1
    )

    :: === 修复3：安全更新当前进程PATH ===
    set "PATH=%dominusprime_BIN%;!PATH!"
    echo [dominusprime] Added %dominusprime_BIN% to PATH
)

REM ──── Done ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
echo.
echo dominusprime installed successfully!
echo.
echo   Install location:  %dominusprime_HOME%
...
echo.
echo   dominusprime init       # first-time setup
echo   dominusprime app        # start dominusprime
echo.
echo To upgrade later, re-run this installer.
echo To uninstall, run: dominusprime uninstall
